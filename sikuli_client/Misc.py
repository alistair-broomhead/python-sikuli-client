...
class SikuliUnreflected(object):
    ...
    def find(self, PS):
        ...
        Pattern.similar() before, a default minimum similarity of 0.7 is
        set automatically.

            If autoWaitTimeout is set to a non-zero value, find() just acts as
        a wait().

            Side Effect lastMatch: the last match can be accessed using
